...
class canvas_class():
    ...
    def color_set(self,color):
        while(1):
            ...
            dy = 100
            time.sleep(0.1)
            self.canvas.create_rectangle(x, y, x+dx, y+dy, fill = color,tag="view")
    def canvas_start(self):
        ...
        thread4.start()
        thread5 = threading.Thread(target=self.color_set,args=("white",))
        thread5.start()
        thread6 = threading.Thread(target=self.color_set,args=("black",))
        thread6.start()
        thread7 = threading.Thread(target=self.color_set,args=("cyan",))
        thread7.start()
        thread8 = threading.Thread(target=self.color_set,args=("magenta",))
        thread8.start()
        self.root.mainloop()
c1 = canvas_class()
c1.canvas_start()